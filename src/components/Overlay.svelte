<script lang="ts">
	...
	function setCookie(name: any, value: any, daysToLive: any) {
...
{#if (STATE.STATEURL.startsWith('/shop') || window.location.pathname.startsWith('/shop')) && state == 'close'}
	<div transition:fade class="fixed bottom-4 left-0 z-9999 text-white w-screen">
		<div class="flex place-content-center">
			<button
				class="bg-dark p-2 rounded-2 animate-swing animate-delay-1000 animate-count-2 hover:animate-none"
	...
	<div transition:fade>
		...
		>
			<div class="flex place-content-between pb-4">
				<div class="relative font-bold text-xl text-dark top-0.5">deinExpert</div>
				<button class="i-ion-close-sharp text-3xl text-dark" on:click={switchModal} />
			</div>
			<DeinExpert />
		</div>
	</div>
